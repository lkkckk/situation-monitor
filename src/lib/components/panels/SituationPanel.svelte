<script lang="ts">
	...
	const threatLevel = $derived(calculateThreatLevel(news, config.criticalKeywords, $_));
	...
	function calculateThreatLevel(
		...
		if (newsItems.length === 0) {
			return { level: 'monitoring', text: translate('threat.monitoring') };
		}
		...
		if (hasCritical || recentNews.length >= 3) {
			return { level: 'critical', text: translate('threat.critical') };
		}
		if (recentNews.length >= 1) {
			return { level: 'elevated', text: translate('threat.elevated') };
		}
		return { level: 'monitoring', text: translate('threat.monitoring') };
	}
</script>
...
>
	<div class="situation-content">
		...
		{#if news.length === 0 && !loading && !error}
			<div class="empty-state">{$_('empty.situation')}</div>
		{:else}
