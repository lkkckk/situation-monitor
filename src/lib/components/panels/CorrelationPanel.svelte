...
<Panel id="correlation" title={$_('panels.correlation')} {loading} {error}>
	{#if news.length === 0 && !loading && !error}
		<div class="empty-state">{$_('correlation.insufficient')}</div>
	{:else if analysis}
		<div class="correlation-content">
			{#if analysis.emergingPatterns.length > 0}
				<div class="section">
					<div class="section-title">{$_('correlation.emerging')}</div>
					{#each analysis.emergingPatterns.slice(0, 3) as pattern}
						<div class="pattern-item">
							...
							<div class="pattern-sources">
								...
								({$_('correlation.items', { values: { count: pattern.count } })})
							</div>
						</div>
					{/each}
				</div>
			...
			{#if analysis.momentumSignals.length > 0}
				<div class="section">
					<div class="section-title">{$_('correlation.momentum')}</div>
			...
			{#if analysis.crossSourceCorrelations.length > 0}
				<div class="section">
					<div class="section-title">{$_('correlation.crossSource')}</div>
					{#each analysis.crossSourceCorrelations.slice(0, 3) as corr}
						<div class="correlation-item">
							...
							<div class="correlation-topic">
								{corr.name} ({$_('correlation.sources', { values: { count: corr.sourceCount } })})
							</div>
						</div>
					{/each}
				</div>
			...
			{#if analysis.predictiveSignals.length > 0}
				<div class="section">
					<div class="section-title">{$_('correlation.predictive')}</div>
					{#each analysis.predictiveSignals.slice(0, 2) as signal}
						<div class="predictive-item">
							...
							<div class="predictive-confidence">
								{$_('correlation.confidence', {
									values: { percent: Math.round(signal.confidence * 100) }
								})}
							</div>
						</div>
					{/each}
			...
			{#if analysis.emergingPatterns.length === 0 && analysis.momentumSignals.length === 0}
				<div class="empty-state">{$_('empty.correlation')}</div>
			{/if}
		</div>
	{:else}
		<div class="empty-state">{$_('empty.correlation')}</div>
